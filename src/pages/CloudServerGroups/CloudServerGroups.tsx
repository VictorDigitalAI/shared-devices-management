...
const CloudServerGroups = () => {
    ...
    useEffect(() => {
        fetchCloudServerGroups();
        // fetchSharedDeviceGroups();
        // fetchSharedDevices();
    }, []);

    const fetchCloudServerGroups = () => {
        setIsLoading(true);
        axios.get(BASE_V1_URL + "/cloud-servers").then(response => {
    ...
    const toolbar = (
        <DotActionToolbar className="table-toolbar" variant="regular">
            <div className="table-title">
                <DotTypography variant="h2">Cloud Server Groups</DotTypography>
            </div>
...
export default CloudServerGroups;
